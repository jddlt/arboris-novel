      ...
      <div class="h-full flex flex-col">
        ...
        <div class="p-4 border-b border-gray-100 flex-shrink-0">
          <div class="flex items-center gap-3 mb-3">
            <div class="w-9 h-9 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
              <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
              </svg>
            </div>
            <div class="min-w-0">
              <h2 class="text-base font-bold text-gray-900">故事蓝图</h2>
              <p class="text-xs text-gray-600 truncate">{{ project.blueprint?.style || '未设定风格' }}</p>
            </div>
          </div>

          <div class="space-y-2">
            <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-2.5">
              <h3 class="text-xs font-semibold text-blue-900 mb-1">故事概要</h3>
              <Tooltip :text="project.blueprint?.one_sentence_summary">
                <p class="text-xs text-blue-700 line-clamp-2">{{ project.blueprint?.one_sentence_summary || '暂无概要' }}</p>
              </Tooltip>
            </div>
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div class="bg-purple-50 rounded-lg p-1.5 text-center">
                <div class="font-semibold text-purple-800">{{ characterCount }}</div>
                <div class="text-purple-600 text-[10px]">角色</div>
              </div>
              <div class="bg-green-50 rounded-lg p-1.5 text-center">
                <div class="font-semibold text-green-800">{{ relationshipCount }}</div>
                <div class="text-green-600 text-[10px]">关系</div>
              </div>
            </div>
          </div>
        </div>
